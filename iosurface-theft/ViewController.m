...
CGDisplayStreamRef _Nullable SLSHWCaptureStreamCreateWithWindow(
    CGWindowID wid, uint32_t /*CGSWindowCaptureOptions*/ options, CFDictionaryRef properties,
    dispatch_queue_t _Nullable queue, CGDisplayStreamFrameAvailableHandler handler);

CGConnectionID SLSMainConnectionID(void);

CGError CGSGetWindowResolution(CGConnectionID cid, CGWindowID wid, CGFloat *_Nonnull resolution,
...
- (void)viewDidLoad {
    ...
    NSLog(@"kFigQuickTimeMetadata_PixelDensityKey_WidthPixels: %@",
          kFigQuickTimeMetadata_PixelDensityKey_WidthPixels);
    NSLog(@"kFigQuickTimeMetadata_PixelDensityKey_HeightPixels: %@",
          kFigQuickTimeMetadata_PixelDensityKey_HeightPixels);
    NSLog(@"kFigQuickTimeMetadata_PixelDensityKey_WidthPoints: %@",
          kFigQuickTimeMetadata_PixelDensityKey_WidthPoints);
    NSLog(@"kFigQuickTimeMetadata_PixelDensityKey_HeightPoints: %@",
          kFigQuickTimeMetadata_PixelDensityKey_HeightPoints);
...
NSImage *fromIOSurface(IOSurfaceRef surface) {
    ...
    NSImage *newImage = [[NSImage alloc] initWithCGImage:img size:ci.extent.size];
    return newImage;
}
...
void compCb(void *outputCallbackRefCon, void *sourceFrameRefCon, OSStatus status,
    ...
    NSLog(@"compPb blockBuffer: sz: %zu bbps: %d lo: %zu tlo: %zu dbo: %p %@", blockBufferLen,
          (int)bbps, lo, tlo, dbo, blockBuffer);
    ...
    NSDictionary *adict = attachments[0];
    bool is_kf          = true;
    if (adict[(__bridge NSString *)kCMSampleAttachmentKey_NotSync] &&
        [adict[(__bridge NSString *)kCMSampleAttachmentKey_NotSync] boolValue] == YES) {
        is_kf = false;
    }
    NSLog(@"is_kf: %d", is_kf);
    ...
    if (is_kf) {
        ...
        size_t ps_cnt = 0;
        OSStatus psres =
            CMVideoFormatDescriptionGetH264ParameterSetAtIndex(fdref, 0, NULL, NULL, &ps_cnt, NULL);
        assert(!psres);
        ...
        for (size_t psi = 0; psi < ps_cnt; ++psi) {
            ...
            psres = CMVideoFormatDescriptionGetH264ParameterSetAtIndex(fdref, psi, &psb, &psb_sz,
                                                                       NULL, &psb_nal_hdr_len);
            NSLog(@"psres: %d psi: %zu psb: %p psb_sz: %zu psb_nal_hdr_len: %d", psres, psi, psb,
                  psb_sz, psb_nal_hdr_len);
            assert(!psres);
...
- (IBAction)captureClicked:(id)sender {
    ...
    for (NSWindow *w in NSApplication.sharedApplication.windows) {
        NSLog(@"window: %@ w: %f h: %f wid: %ld", w, w.frame.size.width, w.frame.size.height,
              (long)w.windowNumber);
        ...
        videoWriter = [[AVAssetWriter alloc] initWithURL:m4v_url
        ...
        //        [videoWriter
        //        startSessionAtSourceTime:CMTimeFromMachAbsoluteTime(mach_absolute_time())];
        ...
        assert(tb);
        NSLog(@"tb: %@", tb);
        OSStatus tbsrs = CMTimebaseSetRateAndAnchorTime(
            tb, 1.0, kCMTimeZero, CMClockMakeHostTimeFromSystemUnits(mach_absolute_time()));
        assert(!tbsrs);
        NSLog(@"tb: %@", tb);
        ...
        OSStatus oss             = VTCompressionSessionCreate(
            NULL, (int32_t)w.frame.size.width * 2, (int32_t)w.frame.size.height * 2,
            kCMVideoCodecType_H264, (__bridge CFDictionaryRef _Nullable)(encSpec),
            (__bridge CFDictionaryRef _Nullable)(srcImgAttr), NULL, compCb,
            (__bridge void *_Nullable)(self), &csref);
        ...
        //        dispatch_queue_t dsq = dispatch_queue_create("iosurface-theft-dsq",
        //        DISPATCH_QUEUE_SERIAL); dispatch_resume(dsq);
        dsref = SLSHWCaptureStreamCreateWithWindow(
            (CGWindowID)w.windowNumber, 0, (__bridge CFDictionaryRef)(dsprops),
            dispatch_get_main_queue(),
            ^(CGDisplayStreamFrameStatus status, uint64_t displayTime,
              IOSurfaceRef _Nullable frameSurface, CGDisplayStreamUpdateRef _Nullable updateRef) {
                IOSurface *fs = (__bridge IOSurface *)frameSurface;
                NSLog(@"callback lol surface: %@ w: %ld h: %ld attachments: %@, update: %@", fs,
                      (long)fs.width, (long)fs.height, fs.allAttachments, updateRef);
                NSDictionary *pbAttr = @{};
                CVPixelBufferRef pbref;
                CVReturn pbres = CVPixelBufferCreateWithIOSurface(
                    NULL, frameSurface, (__bridge CFDictionaryRef _Nullable)(pbAttr), &pbref);
                if (pbres) {
                    NSLog(@"bad pbres: %d\n", pbres);
                    return;
                }
                ...
                OSStatus compFrameStatus = VTCompressionSessionEncodeFrame(
                    self->csref, pbref, my_time, kCMTimeInvalid,
                    (__bridge CFDictionaryRef)frameProps, pbref, &infoFlagsOut);
                NSLog(@"VTCompressionSessionEncodeFrame: %d flags: 0x%x", compFrameStatus,
                      infoFlagsOut);
        ...
        mTimer = [NSTimer scheduledTimerWithTimeInterval:5
                                                  target:self
                                                selector:@selector(finishRecord2:)
                                                userInfo:nil
                                                 repeats:NO];
        NSLog(@"wild end");
    }
...
- (void)finishRecord2:(NSTimer *)timer {
    ...
    //    [videoWriter endSessionAtSourceTime:…]; //optional can call finishWriting without
    //    specifying endTime
